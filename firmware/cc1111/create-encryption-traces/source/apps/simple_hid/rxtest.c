...
void small_delay() {
        int n = 100;	// determined empirically
        while (n--) {
                asm("NOP");
        }
}
...
void loadConfig1() {
  ...
  MCSM1 = 0x30; //0x30;       //Main Radio Control State Machine Configuration 
...
void lowPower() {
  //PA_TABLE1 = 0x03;   //PA Power Setting 1 
  PA_TABLE0 = 0x03;   //PA Power Setting 0 
}
...
//led_blink(3);
  ...
  RFTX;
    ...
    unsigned char keyfixed[16] = {0};
    unsigned char keyvar[16] = {0};
    ...
    for (int i = 0; i < 20000; i++) {
      ...
      if(i%2 ==0) //starting trace will be fixed key
        { AES_init_ctx(&aesContext, keyfixed); }
      else //alternating traces with variable key
        { AES_init_ctx(&aesContext, keyvar); }
